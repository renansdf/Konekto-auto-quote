...
import { IRevisionServices } from '../RevisionDocument';
import { Container, ProductCard } from './styles';

interface IComponentProps {
  options: IRevisionServices;
  whenSelected: () => void;
}
...
const RevisionProducts: React.FC<IComponentProps> = ({
  options,
  whenSelected,
}) => {
  ...
  function generateProductCards(numberOfWords: number, languageGroup: number) {
    if (serviceData) {
      const parsedOptions = options.revisao.map((option) => {
